...
module.exports.run = async (interaction, HubName, BridgedChannel, hubnameStr) => {
  ...
  const created = await createHub(HubName, fittedHubName, interaction.user.id, config.maxAllowedHubs);
  if (created) {
  ...
  } else messageFail(interaction, `Your account already owns a maximum of ${config.maxAllowedHubs} hubs. You can't create more then that!`);
};
